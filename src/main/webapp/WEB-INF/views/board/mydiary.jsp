...
<html lang="kr">
    ...
    <body>
		...
		<div style="overflow: hidden;">
			<div style="margin-top: 100px; position: fixed; left: 10px;">
				<div style="overflow-y: scroll; max-height: 400px;" id="image_container">
					...
					<c:forEach var="diaryDto" items="${diaryList}">
						<c:if test="${diaryDto.diaryNo != null}">
							<img class="btn" style="max-height: 60%; max-width: 60%" alt="${diaryDto.diaryNo}" src="<c:url value='/diaryImg/${diaryDto.imgName}'/>" id="del" title="삭제">
							<button type="button" class="btn btn-secondary" onclick="diaryUpd('${root}', '${diaryDto.diaryNo}')">수정</button>
						</c:if>
					</c:forEach>
				</div>
			</div>
